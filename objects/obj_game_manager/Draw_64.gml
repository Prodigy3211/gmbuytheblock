...
draw_set_colour(c_black);
draw_set_alpha(0.5);
draw_rectangle(10,10,320,165,false); //Background big enough to have cash and progress bar
...
draw_rectangle(20, 128, 200, 138, false) //empty background for a gauge
...
draw_set_colour(c_white);
draw_set_halign(fa_left);
draw_text(210, 124, "Gov. Threat: " +string(floor(global.enemy_threat)) + "%");


//MOVED ORIGINAL BUILDING INSPECTION TO DRAW END EVENT
...
if(game_over_state != "playing") {
	...
	draw_set_halign(fa_center);
	draw_set_color(c_yellow);
	var prompt_text = (game_over_state == "win") ? "[ Press 'R' to Restart || Press 'Q' to Quit ]" : "[Press 'R' to Try Again || Press 'Q' to Quit]";
